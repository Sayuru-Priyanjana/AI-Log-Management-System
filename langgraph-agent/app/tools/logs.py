...
class LogTool:
    ...
    async def warning_samples(self, plan: InvestigationPlan, window: TimeWindow, size: int) -> list[LogSample]:
        """A few raw warning lines from the start of the incident."""
        return await self.fetch(plan, window, levels=["WARN", "WARNING"],
                                order="oldest", limit=size)
    ...
    async def collect(self, plan: InvestigationPlan, incident: TimeWindow,
        ...
        try:
            ...
            evidence.warning_samples = await self.warning_samples(
                plan, incident, size=settings.max_log_patterns
            )
        except OpenSearchError as exc:
