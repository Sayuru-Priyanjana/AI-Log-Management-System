...
is narrow and specific:
  ...
  5. Analyze any warning logs separately and provide your findings in `warning_analysis`.
...
def _format_warning_samples(evidence: EvidenceBundle, limit: int) -> str:
    samples = evidence.logs.warning_samples[:limit]
    if not samples:
        return "No warning logs found in scope."
    lines = []
    for s in samples:
        lines.append(f"[{s.id}] {s.level} {s.service} {clock(s.timestamp)}: {s.message[:200]}")
    return "\n".join(lines)
...
def build_selection_prompt(plan: InvestigationPlan, windows: InvestigationWindows,
    ...
    sections += [
        ...
        "",
        "=== WARNING LOGS ===",
        _format_warning_samples(evidence, settings.max_prompt_patterns),
        "",
        "=== KUBERNETES EVENTS ===",
